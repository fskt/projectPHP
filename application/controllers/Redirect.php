...
class Redirect extends MY_Controller {
	public function index(){
		...
		if($cekCreator!=null){
			...
			$param 					 = (object) $param;
			$qry = $this->UserModel->updateUserData($param);
			
			if($qry!=null){
				/*Youtube*/
				$username = $qry['yt_youtube'];
				$query_youtube = $this->statisticYoutube($username);
				$param = array();
				$param['id_tables']		 = $qry['id_table'];
				...
				$param['y_view'] 		 = $query_youtube['items'][0]['statistics']['viewCount'];
				$param['y_comment'] 	 = $query_youtube['items'][0]['statistics']['commentCount'];
				
				$channelId = $query_youtube['items'][0]['id'];
				$query_youtube = $this->channelYoutube($channelId);
				$param['user_id'] 		 = $query['data']['id'];
				$param['y_data'] 	 	= json_encode($query_youtube['items']);
				
				$param 					 = (object) $param;
				$this->UserModel->dataUpdateMedia($param);
				/*Youtube*/
			}
	...
	private function statisticYoutube($username){
		$path_config = $this->config->item('path_config');
		$url = 'https://www.googleapis.com/youtube/v3/channels?part=statistics&id='.$username.'&key='.$path_config['youtube']['key'];
		$instagramInfo = $this->connectAction($url);
		$results = json_decode($instagramInfo,true);
		return ($results);
	}
	
	private function channelYoutube($channelId){
		$path_config = $this->config->item('path_config');
		$url = 'https://www.googleapis.com/youtube/v3/search?part=snippet&channelId='.$channelId.'&maxResults=5&order=date&key='.$path_config['youtube']['key'];
		$instagramInfo = $this->connectAction($url);
		$results = json_decode($instagramInfo,true);
		return ($results);
	}
	
	private function connectAction($url){
		$ch = curl_init();
